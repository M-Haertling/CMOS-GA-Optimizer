...
public class SAMutate extends SAGeneticOperation{
    ...
    public SAMutate(double mutationRate,double maxPerturbation){
    ...
    public void operate(ChromosomeQueue queue, FittingRoom fr, Chromosome[] in, Chromosome[] out, boolean fit, boolean deduct) {
        ...
        //Do SA things
        if(in[0].getFitness()>out[0].getFitness()){
            //Decide if the new chromosome should be kept
            if(!this.shouldAcceptChild(in[0].getFitness(),out[0].getFitness())){
                //Decline the child
                in[0].copyAllTo(out[0]);
            }
        }
        
//        System.out.println("SAMutate: "+out[0].getFitness());
    }
